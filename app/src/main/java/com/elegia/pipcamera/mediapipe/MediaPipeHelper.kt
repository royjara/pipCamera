...
class MediaPipeHelper(
    ...
    val context: Context,
    val mediaPipeHelperListener: MediaPipeListener? = null
) {
    ...
    fun setupFaceLandmarker() {
        ...
        when (runningMode) {
            RunningMode.LIVE_STREAM -> {
                if (mediaPipeHelperListener == null) {
                    throw IllegalStateException(
                        "mediaPipeHelperListener must be set when runningMode is LIVE_STREAM."
                    )
                }
            }
            else -> {
        ...
        try {
            ...
            mediaPipeHelperListener?.onError(
    ...
    private fun returnLivestreamResult(
        ...
        if (result.faceLandmarks().size > 0) {
            ...
            mediaPipeHelperListener?.onResults(
                ResultBundle(
                    result,
                    inferenceTime,
                    input.height,
            ...
            mediaPipeHelperListener?.onEmpty()
        }
    }
